...
void ReportConfigProperties()
{
	UpdateReportedProperties(
		"{ 'Config': { 'TelemetryInterval': %d }, 'FirewareVersion': '2.0'}",
		g_telemetryInterval);
}
...
{
	UpdateReportedProperties("{ 'SupportedMethods': { 'LightBlink': 'light blink', 'ChangeLightStatus--LightStatusValue-int': 'Change light status, 0 off 1 on', 'InitiateFirmwareUpdate--FwPackageUri-string': 'Updates device Firmware. Use parameter FwPackageUri to specifiy the URI of the firmware file, e.g. https://iotrmassets.blob.core.windows.net/firmwares/FW20.bin' } }");
}
